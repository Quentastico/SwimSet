...
class Segment:
  ...
  def setStroke(self):
    self.stroke = strokeTypes[np.random.randint(low = 0, high = len(strokeTypes))]
  ...
  def setEquipment(self):
    self.equipment = equipmentTypes[np.random.randint(low = 0, high = len(equipmentTypes))]

  # Method to set the intensity
  # Same thng: Maybe have a think of when the intensity makes sense...
  def setIntensity(self):
    self.intensity = np.random.randint(low = minIntensity, high = maxIntensity + 1)
